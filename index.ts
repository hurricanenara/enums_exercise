...
type ColorKey = Partial<keyof typeof Color>;
...
type StatusObject = {
  ...
  // string 말고 다른걸 써서 되게 하면 key 부분이 에러가 생기는 상황..
};
...
export function getCars(): Car[] {
  return [Car.Coupe]  // 반환 타입을 반환하라는게 무슨 말이지
}
...
export function getProgrammingLanguages() {
  return Object.entries(ProgrammingLanguage).map((e) => {
    let test: TProgrammingLanguages = {};
    test[e[0].length] = e[0];
    return test;
  })
}
