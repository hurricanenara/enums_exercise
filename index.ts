...
export function isStatusPending(status: Status.Pending): string {
  return status;
}

export function isStatusComplete(status: Status.Pending): string {
  return status;
}
...
export function getStatusObject(status: StatusObject) {
  return status;
}
...
export function getCars(): Car[Color] {
  const carColor: Color = "Red" as Color;
  return carColor;
}
